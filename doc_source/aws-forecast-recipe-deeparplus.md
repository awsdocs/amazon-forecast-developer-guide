...
| prediction\_length |  The number of time\-steps that the model is trained to predict, also called the forecast horizon\. The trained model always generates forecasts with this length\. The `prediction_length` is set with the `ForecastHorizon` parameter of the [CreatePredictor](API_CreatePredictor.md) API\. It cannot be changed without retraining the model\. [\[See the AWS documentation website for more details\]](http://docs.aws.amazon.com/forecast/latest/dg/aws-forecast-recipe-deeparplus.html)  | 
